    ...
    public partial class StockAdjustment : Form
    {
        ...
        private void DGVStockUdjustment_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (DGVStockUdjustment.Columns[e.ColumnIndex].Name == "colUpdate")
            {
                try
                {
                    double UnitCost = Convert.ToDouble(DGVStockUdjustment.Rows[e.RowIndex].Cells["colNewUnitCost"].Value);
                    double UnitPrice = Convert.ToDouble(DGVStockUdjustment.Rows[e.RowIndex].Cells["colNewUnitPrice"].Value);
                    int inStockQTY = Convert.ToInt32(DGVStockUdjustment.Rows[e.RowIndex].Cells["colNewInStock"].Value);
